...
本关任务：根据 C1 语言的 EBNF 描述（见[关卡1](part1.md#c1-语言的文法)），修改并完善本实训版本库中 C1 语言的语法描述文件`grammar/C1Parser.g4`，编译和运行生成的语法分析器。请：
...
1. 阅读理解版本库中提供的 C1 语言的词法描述文件`grammar/C1Lexer.g4`，建立与语法描述文件`grammar/C1Parser.g4`之间的关系；
2. 修改并完善版本库中 C1 语言的语法描述文件`grammar/C1Parser.g4`；
3. 用antlr4生成分析器源码到build目录下(参考第2关编译和运行以及可视化)，理解`grammar/C1Parser.g4`与生成的分析器源码之间的关系；
4. 测试输入应置于`test/parser`目录下：
   - 正确测试用例文件名前缀为`pt_`；
   - 错误测试用例文件名前缀为`ft_`。
5. 在[`doc/labLexerParser.md`](./labLexerParser.md)中编写对问题`Parser-Q1` ~ `Parser-Q2`的回答，描述实验遇到的问题。
...
语法分析的语法文件（未完成）在`grammar/C1Parser.g4`。它的初始内容如下：
...
你可以使用如下指令生成C1程序的解析树（命令行展示或者图形界面展示）。
...
grun C1 compilationUnit -tree ../test/demo/simple.c1
# inspect the parse tree in GUI
grun C1 exp -gui ../test/ast/exp.c1
```

这里`simple.c1`和`exp.c1`是预置的简易测试文件。
...
在完成实验时，你可以指定不同的`startRuleName`(例如`exp`)来逐步增强你的语法分析器的能力。例如你可以通过`grun C1 exp -tree ../test/exp/exp.c1`来解析不完整的C1程序。逐步解析并充分考虑每一步解析的正确性可以帮助你快速构建鲁棒的语法分析器。
